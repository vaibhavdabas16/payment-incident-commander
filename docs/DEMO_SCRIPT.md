...
1. Open `http://127.0.0.1:8000` and let it warm up (~20s of simulated traffic).
2. Go to **Overview → Load demo history**. This seeds fourteen labelled historical records so the
   learning section has something to have learned from. Say so on camera when you get there — they
   are marked *seeded* in the UI and you should not pretend otherwise.
3. Go to **Simulate**, and have *UPI PSP route degradation* ready to click.
4. Deterministic reasoner is the default and is what the benchmark uses; leave it.
